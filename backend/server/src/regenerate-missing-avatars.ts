...
async function injectAiPrompts(bookPath: string, generatedPromptsPath: string): Promise<void> {
  ...
  for (const prompt of generatedPrompts.characters) {
    ...
    await updateCharacterFolder({ bookPath, characterSlug: prompt.name, displayName: matchingFolder.displayName, summary: matchingFolder.summary, aiPrompt: prompt.visualGuide });
    injected++;
  }
...
async function findCharactersMissingAvatars(bookPath: string): Promise<MissingAvatars> {
  ...
  if (missingWebp.length > 0) {
    ...
    missingWebp.forEach((slug) => {
      console.log(`  npx convex run admin/regenerateAvatarWebp:regenerateAvatarWebp '{"characterPath": "${bookPath}/characters/${slug}"}'`);
    });
  }
...
async function generateSingleAvatar(
  ...
  try {
    await convex.markCharacterAvatarState({ characterPath: `${bookPath}/characters/${character.slug}`, state: "generating" });

    const imageBuffer = await generateCharacterImageWithOpenAI(character.aiPrompt, character.displayName, avatarStyle);

    if (!imageBuffer) {
      await convex.markCharacterAvatarState({ characterPath: `${bookPath}/characters/${character.slug}`, state: "error" });
    ...
    await convex.markCharacterAvatarState({ characterPath: `${bookPath}/characters/${character.slug}`, state: "ready" });
...
async function regenerateMissingAvatars(bookPath: string, avatarStyle: string): Promise<void> {
  ...
  const results = await Promise.all(missingLarge.map((character) => generateSingleAvatar(bookPath, character, avatarStyle)));
...
async function main() {
  ...
  if (args.length < 1) {
    ...
    console.log("  tsx regenerate-missing-avatars.ts 1766867074631-dolega-mostowicz-prokurator-alicja-horn --inject-only");
    console.log("  tsx regenerate-missing-avatars.ts 1766867074631-dolega-mostowicz-prokurator-alicja-horn --check-only");
    process.exit(1);
  }
  ...
  const generatedPromptsPath = path.resolve(__dirname, `../../books-data/${bookSlug}/temporary-output/generated-prompts.json`);
  ...
  const graphicalStylePath = path.resolve(__dirname, `../../books-data/${bookSlug}/temporary-output/graphicalStyle.json`);
