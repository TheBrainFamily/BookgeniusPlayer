...
export function markStepComplete(slug: string, step: Step, startedAt: number, endedAt: number): void {
  const progress = readProgress(slug) || initProgress(slug);
  progress.completedSteps[step] = { status: "done", startedAt: new Date(startedAt).toISOString(), endedAt: new Date(endedAt).toISOString() };
...
export function markStepError(slug: string, step: Step, error: string, startedAt: number, endedAt: number): void {
  const progress = readProgress(slug) || initProgress(slug);
  progress.completedSteps[step] = { status: "error", startedAt: new Date(startedAt).toISOString(), endedAt: new Date(endedAt).toISOString(), error };
  progress.lastAttemptedStep = step;
  writeProgress(slug, progress);
}
...
export function createRetroactiveProgress(slug: string, lastCompletedStep: Step): PipelineProgress {
  ...
  const progress: PipelineProgress = { slug, startedAt: now, updatedAt: now, completedSteps: {}, lastCompletedStep, lastAttemptedStep: null };
