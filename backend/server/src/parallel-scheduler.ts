...
export function getReadySteps(allSteps: Step[], completedSteps: Set<Step>, runningSteps: Set<Step>): Step[] {
  return allSteps.filter((step) => !completedSteps.has(step) && !runningSteps.has(step) && canRunStep(step, completedSteps));
}

export type ParallelSchedulerState = { completedSteps: Set<Step>; runningSteps: Set<Step>; failedSteps: Set<Step>; stepPromises: Map<Step, Promise<void>> };
