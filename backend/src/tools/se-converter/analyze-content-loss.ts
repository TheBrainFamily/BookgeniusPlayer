  ...
  try {
    ...
    for (const file of xhtmlFiles) {
      ...
      if (/interlude/i.test(file)) {
        ...
        if (paragraphs > 2) {
          risks.push({
            type: "INTERLUDE_CONTENT",
            severity: "CRITICAL",
            description: `Interlude with ${paragraphs} paragraphs needs own chapter`,
            files: [file],
            examples: []
          })
        }
      ...
      if (preambleMatch) {
        risks.push({
          type: "PREAMBLE_BEFORE_NESTED",
          severity: "HIGH",
          description: "Content before nested chapters may be lost during promotion",
          files: [file],
          examples: []
        })
      }
      ...
      if (poemBlocks && poemBlocks.length > 0) {
        ...
        if (existing) {
          ...
          risks.push({
            type: "POETRY_BLOCKS",
            severity: "MEDIUM",
            description: `${poemBlocks.length} poem blocks need line formatting`,
            files: [file],
            examples: []
          })
        }
      ...
      if (figures) {
        ...
        if (existing) {
          ...
          risks.push({
            type: "FIGURES_IMAGES",
            severity: "MEDIUM",
            description: `${figures.length} figures with images`,
            files: [file],
            examples: []
          })
        }
      ...
      if (dataTables) {
        ...
        if (existing) {
          ...
          risks.push({
            type: "DATA_TABLES",
            severity: "MEDIUM",
            description: `${dataTables.length} data tables`,
            files: [file],
            examples: []
          })
        }
        ...
        if (stageCount > 0) {
          ...
          if (existing) {
            ...
            risks.push({
              ...
              severity: "LOW",
              description: `${stageCount} stage directions need italic styling`,
              files: [file],
              examples: []
            })
          }
...
async function main() {
  ...
  const lines = [
    ...
    ``,
    `Risk types:`,
    ...Object.entries(riskCounts).sort((a, b) => b[1] - a[1]).map(([t, c]) => `  ${t}: ${c}`),
    ``,
    `================================================================================`,
  ...
  for (const book of byCategory.HIGH) {
    lines.push(`${book.slug}: ${book.risks.filter(r => r.severity === "HIGH").map(r => r.type).join(", ")}`)
  }
  ...
  console.log(`\nRisk breakdown:`)
  Object.entries(riskCounts).sort((a, b) => b[1] - a[1]).forEach(([t, c]) => console.log(`  ${t}: ${c}`))
}
