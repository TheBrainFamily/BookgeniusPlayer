            ...
            <div className="space-y-2">
              {["View Collection", "My Progress", "Reading History", "Settings"].map((link) => (
                <Button 
                  key={link}
                  variant="ghost" 
                  className="p-0 h-auto text-muted-foreground hover:text-library-gold text-left justify-start"
                >
                  {link}
                </Button>
              ))}
          ...
          <div className="flex space-x-6 mt-4 md:mt-0">
            {["Privacy Policy", "Terms of Service", "Cookie Policy"].map((link) => (
              <Button 
                key={link}
                variant="ghost" 
                className="p-0 h-auto text-muted-foreground hover:text-library-gold text-sm"
              >
                {link}
              </Button>
            ))}
