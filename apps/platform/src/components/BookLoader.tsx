    ...
    <div className={`book-loader-splash ${isHiding ? "hide" : ""} relative px-4 sm:px-6 lg:px-8`}>
      <div className="text-center space-y-6 sm:space-y-8 lg:space-y-10 max-w-4xl mx-auto">
        <div className="mb-6 sm:mb-8 lg:mb-16">
          <h1 className="splash-title text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl">
            {title}
          </h1>
          <h2 className="splash-subtitle text-sm sm:text-base md:text-lg lg:text-xl mt-2 sm:mt-3">
            {author}
          </h2>
        </div>
        ...
        <div className="h-6 sm:h-8 lg:h-10" aria-live="polite">
          <p
            className={`splash-loading-text text-sm sm:text-base lg:text-lg px-2 sm:px-4 ${isFading ? "fading-out" : "fading-in"}`}
          >
            {currentPhrase}
          </p>
        </div>
        ...
        <div className="mt-6 sm:mt-8 lg:mt-16 h-12 sm:h-14 lg:h-16 flex items-center justify-center">
          <button
            ...
            `}
            style={{
              animation: showStartButton ? "buttonPulse 4s ease-in-out infinite" : "none",
