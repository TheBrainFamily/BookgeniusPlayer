...
function formatRange(startChapter: number, startParagraph: number, endChapter: number, endParagraph: number): string {
  if (endChapter === 999) {
...
export const EditorToolbar: React.FC = () => {
  ...
  const { currentBackground, currentMusic, currentChapter, currentParagraph } = useCurrentMediaCues();
      ...
      {isEditModeActive && (
        ...
        >
          <div className="flex items-center gap-3">
            ...
            {currentBackground ? (
              <div className="flex items-center gap-2">
                {currentBackground.previewUrl && <img src={currentBackground.previewUrl} alt="" className="w-8 h-8 rounded object-cover" />}
                <div className="flex flex-col">
                  <span className="text-white text-sm font-medium truncate max-w-[120px]">{currentBackground.fileBasename}</span>
                  <span className="text-zinc-500 text-xs">
                    {formatRange(currentBackground.startChapter, currentBackground.startParagraph, currentBackground.endChapter, currentBackground.endParagraph)}
                  </span>
                </div>
                <button onClick={handleEditBackground} className="p-1.5 rounded bg-zinc-700 hover:bg-zinc-600 text-white transition-colors" title="Edit background">
                  <Edit size={14} />
                </button>
              </div>
            ) : (
            ...
            <button onClick={handleAddBackground} className="p-1.5 rounded bg-purple-600 hover:bg-purple-500 text-white transition-colors" title="Add background here">
              <Plus size={14} />
            </button>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            {currentMusic ? (
              <div className="flex items-center gap-2">
                {currentMusic.coverUrl && <img src={currentMusic.coverUrl} alt="" className="w-8 h-8 rounded object-cover" />}
                <div className="flex flex-col">
                  <span className="text-white text-sm font-medium truncate max-w-[120px]">{currentMusic.title || currentMusic.fileBasename}</span>
                  <span className="text-zinc-500 text-xs">{formatRange(currentMusic.chapter, currentMusic.paragraph, currentMusic.endChapter, currentMusic.endParagraph)}</span>
                </div>
                <button onClick={handleEditMusic} className="p-1.5 rounded bg-zinc-700 hover:bg-zinc-600 text-white transition-colors" title="Edit music">
                  <Edit size={14} />
                </button>
              </div>
            ) : (
            ...
            <button onClick={handleAddMusic} className="p-1.5 rounded bg-purple-600 hover:bg-purple-500 text-white transition-colors" title="Add music here">
              <Plus size={14} />
            </button>
          </div>
        </motion.div>
