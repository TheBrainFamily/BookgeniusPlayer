...
const buildParagraphCache = (): ParagraphInfo[] => {
  ...
  return Array.from(nodes).map((p) => {
    const chapter = parseInt(
      (p.closest("section[data-chapter]") as HTMLElement).dataset.chapter || "0",
      10,
    );
...
const DeepResearchMarkdown: React.FC<DMProps> = ({ text, className = "" }) => {
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      ...
      goToParagraph(
        { currentChapter: chapter, currentParagraph: paragraph },
        { behavior: "smooth" },
      ).catch((error) => console.warn("Failed to navigate to paragraph from markdown:", error));
    };
  ...
  const components = {
    h1: (props: ComponentPropsWithoutRef<"h1">) => (
      <h1
        {...props}
        className="mt-12 mb-6 text-3xl font-extrabold border-b border-white pb-3 tracking-tight first:mt-0"
      />
    ),
    h2: (props: ComponentPropsWithoutRef<"h2">) => (
      <h2
        {...props}
        className="mt-10 mb-4 text-2xl font-semibold border-l-4 border-primary pl-4 first:mt-0"
      />
    ),
    h3: (props: ComponentPropsWithoutRef<"h3">) => (
      <h3 {...props} className="mt-8 mb-3 text-xl font-medium text-white first:mt-0" />
    ),
    p: (props: ComponentPropsWithoutRef<"p">) => (
      <p {...props} className="leading-relaxed text-white/90 max-w-none prose-p:leading-normal" />
    ),
    ...
    blockquote: (props: ComponentPropsWithoutRef<"blockquote">) => (
      <blockquote
        {...props}
        className="border-l-4 border-primary/80 italic pl-5 my-4 text-white/70"
      />
    ),
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    li: ({ ordered, ...props }: ListItemProps) => (
      <li {...props} className="pl-1 marker:font-semibold" />
    ),
    code: ({ inline, children, ...props }: CodeProps) => {
      if (inline) {
        return (
          <code
            {...props}
            className="px-1 py-0.5 rounded bg-gray-100 text-pink-600 font-mono text-sm"
          >
            {children}
          </code>
        );
  ...
  return (
    <motion.article
      ...
      className={
        "prose prose-neutral max-w-none lg:prose-lg xl:prose-xl dark:prose-invert mx-auto" +
        (className ? ` ${className}` : "")
      }
    >
      <ReactMarkdown
