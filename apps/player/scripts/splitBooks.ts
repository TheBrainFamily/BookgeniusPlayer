...
function splitBook(bookFolder: string) {
  const bookXmlPath = path.join(bookFolder, "book.xml");
  ...
  const metadataContent = content.substring(0, firstChapterIndex) + "</ebook>";

  // Create booksContent folder
  const contentDir = path.join(bookFolder, "booksContent");
  fs.mkdirSync(contentDir, { recursive: true });

  fs.writeFileSync(path.join(contentDir, "metadata.xml"), metadataContent, "utf8");
  console.log(`Created metadata.xml in ${contentDir}`);
