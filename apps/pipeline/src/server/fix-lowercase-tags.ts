...
function findLowercaseCharacterTags(xml: string, validCharacterTags: string[]): TagReplacement[] {
  ...
  for (const [lowercaseTag, count] of lowercaseTagCounts) {
    const matchingCharacter = validCharacterTags.find(
      (charTag) => charTag.toLowerCase() === lowercaseTag.toLowerCase(),
    );
...
export async function fixLowercaseTags(
  ...
  const validCharacterTags = characterCards.map((c) => generateTagName(c.name) as string);
