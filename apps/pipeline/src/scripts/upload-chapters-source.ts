...
function parseArgs(): CliArgs {
  ...
  if (args.length < 2) {
    console.error(
      "Usage: bun apps/pipeline/src/scripts/upload-chapters-source.ts <book-slug> <file-or-dir> [--only <basename>] [--basename <basename>] [--concurrency <n>] [--dry-run] [--allow-new]",
    );
    process.exit(1);
  }
  ...
  const rawConcurrency = concurrencyIdx !== -1 ? args[concurrencyIdx + 1] : undefined;
  const parsedConcurrency = rawConcurrency ? Number.parseInt(rawConcurrency, 10) : 10;
  const concurrency =
    Number.isFinite(parsedConcurrency) && parsedConcurrency > 0 ? parsedConcurrency : 10;
...
export async function uploadChaptersSource(
  ...
  const concurrencyRaw = args.concurrency ?? 10;
  const concurrency = Number.isFinite(concurrencyRaw) && concurrencyRaw > 0 ? concurrencyRaw : 10;
  ...
  const queue: Array<{ source: string; basename: string }> = [];
...
async function main() {
  const { bookSlug, inputPath, only, basename, concurrency, dryRun, allowNew } = parseArgs();
