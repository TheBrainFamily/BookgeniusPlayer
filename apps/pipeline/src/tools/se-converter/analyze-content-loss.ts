  ...
  try {
    ...
    for (const file of xhtmlFiles) {
      ...
      const preambleMatch = content.match(
        /<section[^>]*>[\s\S]*?<p[^>]*>[^<]{30,}[\s\S]*?<section[^>]*epub:type="[^"]*chapter/g,
      );
      if (preambleMatch) {
        risks.push({
      ...
      const poemBlocks = content.match(
        /<(?:blockquote|div|section)[^>]*epub:type="[^"]*z3998:(?:poem|verse|song)[^"]*"[\s\S]*?<\/(?:blockquote|div|section)>/g,
      );
