...
if [[ "$*" == *"--with-books"* ]]; then
  s5cmd --numworkers 256 cp -c 256 --cache-control "public, max-age=31536000, immutable"  build/s3-data/assets/ "s3://$S3_BUCKET/$CTX/assets/"
  ...
  aws s3 cp build/s3-data/versions.json s3://$S3_BUCKET/$CTX/versions.json \
