...
async function prepareBuild() {
  try {
    ...
    const playerBooksDir = path.join(apps.find(app => app.name === 'player')!.sourceDir, 'books');
    if (await pathExists(playerBooksDir)) {
      ...
      await rename(playerBooksDir, booksTarget);
    } else {
    ...
    for (const app of apps) {
      if (await pathExists(app.sourceDir)) {
        ...
        // The source `dist` folder no longer contains `books`, as we moved it.
        // This will copy the rest of the application shell.
