...
function processParagraph(
  ...
  const processInlineElement = (element: Element, characterMap: Map<string, CharacterInfo>, bookSlug: BOOK_SLUGS): string => {
    ...
    switch (element.tagName) {
      ...
      case "musicShift":
        return `<span class="absolute top-0 right-5 group" data-editor-mode="true">
                      <span class="cursor-pointer" data-editor-tag="musicShift">♪</span>
                      <span class="hidden group-hover:block absolute right-0 top-6 bg-gray-800 text-white p-2 rounded text-sm whitespace-nowrap">${element.getAttribute("style") || "Music shift"}</span>
                    </span>`;
      case "backgroundShift":
        return `<span class="absolute top-0 right-5 group" data-editor-mode="true">
                      <span class="cursor-pointer" data-editor-tag="backgroundShift">🖼️</span>
                      <span class="hidden group-hover:block absolute right-0 top-6 bg-gray-800 text-white p-2 rounded text-sm whitespace-nowrap">${element.getAttribute("style") || "Background shift"}</span>
                    </span>`;
      default:
  ...
  let jsxContent = `${indent}<${tagName} data-index="${dataIndex}" class="relative">`;
