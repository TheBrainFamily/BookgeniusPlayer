import React from 'react';

export const Chapter11: React.FC = () => {
  return (
    <section data-chapter="11">
      <p data-index="0">
        Rozdział XI
      </p>
      <p data-index="1">
        Skończył się miesiąc Tot i zaczynał miesiąc Paofi, druga połowa lipca. Woda Nilu z zielonawej zrobiła się białą, a potem czerwoną i wciąż przybierała. Królewski wodowskaz w Memfisie był zapełniony prawie na wysokość dwu ludzi, a Nil rósł co dzień na dwie pięści. Najniższe grunta były zalane, z wyższych spiesznie zbierano len, winogrona i pewien rodzaj bawełny. Gdzie z rana było jeszcze sucho, tam ku wieczorowi pluskały fale.
      </p>
      <p data-index="2">
        Zdawało się, że gwałtowny, choć niewidzialny wicher dmie w głębi rzeki. Orze na niej szerokie zagony, wypełnia pianą bruzdy, potem na chwilę wygładza powierzchnię wody, a po chwili skręca ją w przepaściste wiry.
      </p>
      <p data-index="3">
        Znowu orze, znowu wygładza, skręca, napędza nowe góry wody, nowe smugi pian i wciąż podnosi szeleszczącą rzekę, wciąż zdobywa nowe płaty ziemi. Niekiedy woda dosięgnąwszy pewnej granicy, przekracza ją, w oka mgnieniu wlewa się w nizinę i tworzy błyszczące jeziorko tam, gdzie przed chwilą rozsypywały się w proch zwiędłe trawy.
      </p>
      <p data-index="4">
        Choć przybór dosięgnął ledwie trzeciej części swej miary, już całe wybrzeże było zalane. Co godzinę jakiś folwarczek na wzgórzu robił się podobnym do wyspy, z początku odgraniczonej od innych tylko wąskim kanałem, który stopniowo rozszerzał się i coraz bardziej odcinał domostwo od sąsiadów. Nieraz, kto wyszedł do pracy piechotą, wracał czółnem.
      </p>
      <p data-index="5">
        Łódek i tratew ukazywało się na Nilu coraz więcej. Z jednych łapano ryby w sieci, na innych przewożono zbiory do stodół albo ryczące bydło do obór, na innych odwiedzano znajomych, ażeby wśród śmiechu i krzyku zawiadomić ich (na co patrzyli wszyscy), że Nil przybiera. Niekiedy łodzie, skupione jak stado kaczek, rozbiegały się na wszystkie strony przed szeroką tratwą, która z Górnego Egiptu niosła w dół olbrzymie bryły kamienne, wyrąbane w nadbrzeżnych kopalniach.
      </p>
      <p data-index="6">
        W powietrzu, jak ucho sięgło, rozlegał się szelest przybierającej wody, krzyk spłoszonego ptactwa i wesołe śpiewy ludzkie. Nil przybiera, będzie dużo chleba!
      </p>
      <p data-index="7">
        Przez cały ten miesiąc toczyło się śledztwo w sprawie napadu na dom <span 
          className="character-highlighted" 
          data-character="Ksiaze-Ramzes" 
          data-src-listening="/Pharaon/ksiaze-ramzes-listens.mp4"
        >
          następcy tronu
        </span>. Każdego ranka łódź z urzędnikami i milicją przybijała do jakiegoś folwarku. Odrywano ludzi od pracy, zasypywano ich podstępnymi pytaniami, bito kijem. Ku wieczorowi zaś wracały do Memfisu dwie łódki: jedna niosła urzędników, druga więźniów.
      </p>
      <p data-index="8">
        Tym sposobem wyłowiono kilkuset przestępców, z których połowa nie wiedziała o niczym, połowie zaś groziło więzienie lub kilka lat pracy w kamieniołomach. Niczego jednak nie dowiedziano się ani o przewódcach napadu, ani o owym kapłanie, który skłonił lud do rozejścia.
      </p>
      <p data-index="9">
        W księciu <span 
          className="character-highlighted" 
          data-character="Ksiaze-Ramzes" 
          data-src-listening="/Pharaon/ksiaze-ramzes-listens.mp4"
        >
          Ramzesie
        </span> kojarzyły się niezwykle sprzeczne przymioty. Był on gwałtowny jak lew i uparty jak wół. Obok tego miał wielki rozum i głębokie poczucie sprawiedliwości.
      </p>
      <p data-index="10">
        Widząc, że śledztwo prowadzone przez urzędników nie wydaje rezultatu, <span 
          className="character-highlighted" 
          data-character="Ksiaze-Ramzes" 
          data-src-listening="/Pharaon/ksiaze-ramzes-listens.mp4"
        >
          książę
        </span> pewnego dnia sam popłynął do Memfisu i kazał sobie otworzyć więzienie.
      </p>
      <p data-index="11">
        Było ono zbudowane na wzgórzu, otoczone wysokim murem i składało się z wielkiej liczby budynków kamiennych, ceglanych i drewnianych. Budowle te po większej części były tylko wejściami lub mieszkaniami dozorców. Więźniowie zaś mieścili się w podziemnych jaskiniach wykutych w wapiennej skale.
      </p>
      <p data-index="12">
        Kiedy <span 
          className="character-highlighted" 
          data-character="Ksiaze-Ramzes" 
          data-src-listening="/Pharaon/ksiaze-ramzes-listens.mp4"
        >
          następca
        </span> przekroczył bramę, spostrzegł gromadkę kobiet, które myły i karmiły jakiegoś więźnia. Nagi ten człowiek, podobny do szkieletu, siedział na ziemi, trzymając ręce i nogi w czterech otworach kwadratowej deski, która zastępowała kajdany.
      </p>
      <p data-index="13">
        <span 
          className="character-placeholder character-talking" 
          data-character="Ksiaze-Ramzes" 
          data-src-talking="/Pharaon/ksiaze-ramzes-speaks.mp4" 
          data-is-talking="true"
        />\n    — Dawno ten człowiek tak cierpi? — zapytał <span 
          className="character-highlighted" 
          data-character="Ksiaze-Ramzes" 
          data-src-listening="/Pharaon/ksiaze-ramzes-listens.mp4"
        >
          książę
        </span>.\n  
      </p>
      <p data-index="14">
        \n    — Dwa miesiące — odparł nadzorca.\n  
      </p>
      <p data-index="15">
        <span 
          className="character-placeholder character-talking" 
          data-character="Ksiaze-Ramzes" 
          data-src-talking="/Pharaon/ksiaze-ramzes-speaks.mp4" 
          data-is-talking="true"
        />\n    — A długo jeszcze ma siedzieć?\n  
      </p>
      <p data-index="16">
        \n    — Miesiąc.\n  
      </p>
      <p data-index="17">
        <span 
          className="character-placeholder character-talking" 
          data-character="Ksiaze-Ramzes" 
          data-src-talking="/Pharaon/ksiaze-ramzes-speaks.mp4" 
          data-is-talking="true"
        />\n    — Cóż on zrobił?\n  
      </p>
      <p data-index="18">
        \n    — Zelżył urzędnika zbierającego podatki.\n  
      </p>
      <p data-index="19">
        <span 
          className="character-highlighted" 
          data-character="Ksiaze-Ramzes" 
          data-src-listening="/Pharaon/ksiaze-ramzes-listens.mp4"
        >
          Książę
        </span> odwrócił się i ujrzał drugą gromadę, złożoną z kobiet i dzieci. Między nimi był stary człowiek.\n  
      </p>
      <p data-index="20">
        <span 
          className="character-placeholder character-talking" 
          data-character="Ksiaze-Ramzes" 
          data-src-talking="/Pharaon/ksiaze-ramzes-speaks.mp4" 
          data-is-talking="true"
        />\n    — Czy to są więźniowie?\n  
      </p>
      <p data-index="21">
        \n    — Nie, najdostojniejszy panie. To jest rodzina oczekująca na zwłoki przestępcy, który ma być uduszony… O, już prowadzą go do izby… — mówił nadzorca.\n  
      </p>
      <p data-index="22">
        Po czym zwróciwszy się do gromadki rzekł:
      </p>
      <p data-index="23">
        \n    — Bądźcie jeszcze chwilkę cierpliwi, kochani ludzie, zaraz dostaniecie ciało.\n  
      </p>
      <p data-index="24">
        \n    — Bardzo dziękujemy ci, zacny panie — odparł stary człowiek, zapewne ojciec delikwenta. — Wyszliśmy z domu wczoraj wieczór, len został nam w polu, a tu rzeka przybiera!…\n  
      </p>
      <p data-index="25">
        <span 
          className="character-highlighted" 
          data-character="Ksiaze-Ramzes" 
          data-src-listening="/Pharaon/ksiaze-ramzes-listens.mp4"
        >
          Książę
        </span> pobladł i zatrzymał się.\n  
      </p>
      <p data-index="26">
        <span 
          className="character-placeholder character-talking" 
          data-character="Ksiaze-Ramzes" 
          data-src-talking="/Pharaon/ksiaze-ramzes-speaks.mp4" 
          data-is-talking="true"
        />\n    — Wiesz — zwrócił się do nadzorcy — że mam prawo łaski?\n  
      </p>
      <p data-index="27">
        \n    — Tak, <span 
          className="character-highlighted" 
          data-character="Ksiaze-Ramzes" 
          data-src-listening="/Pharaon/ksiaze-ramzes-listens.mp4"
        >
          erpatre
        </span> — odpowiedział nadzorca, kłaniając się, a potem dodał: — Według praw, na pamiątkę twojej bytności w tym miejscu, synu słońca, dobrze prowadzący się, a skazani za obrazę religii lub państwa powinni otrzymać ulgi. Spis tych ludzi będzie złożony u stóp waszych w ciągu miesiąca.\n  
      </p>
      <p data-index="28">
        <span 
          className="character-placeholder character-talking" 
          data-character="Ksiaze-Ramzes" 
          data-src-talking="/Pharaon/ksiaze-ramzes-speaks.mp4" 
          data-is-talking="true"
        />\n    — A ten, którego mają w tej chwili dusić, czy nie ma prawa korzystać z mojej łaski?\n  
      </p>
      <p data-index="29">
        Nadzorca rozłożył ręce i pochylił się w milczeniu.
      </p>
      <p data-index="30">
        Ruszyli z miejsca i przeszli kilka dziedzińców. W drewnianych klatkach, na gołej ziemi, roili się w ciasnocie przestępcy skazani na więzienie. W jednym budynku rozlegały się straszne krzyki: bito dla wydobycia zeznań.
      </p>
      <p data-index="31">
        <span 
          className="character-placeholder character-talking" 
          data-character="Ksiaze-Ramzes" 
          data-src-talking="/Pharaon/ksiaze-ramzes-speaks.mp4" 
          data-is-talking="true"
        />\n    — Chcę zobaczyć oskarżonych o napad na mój dom — rzekł głęboko wzruszony <span 
          className="character-highlighted" 
          data-character="Ksiaze-Ramzes" 
          data-src-listening="/Pharaon/ksiaze-ramzes-listens.mp4"
        >
          następca
        </span>.\n  
      </p>
      <p data-index="32">
        \n    — Jest ich z górą trzystu — odparł nadzorca.\n  
      </p>
      <p data-index="33">
        <span 
          className="character-placeholder character-talking" 
          data-character="Ksiaze-Ramzes" 
          data-src-talking="/Pharaon/ksiaze-ramzes-speaks.mp4" 
          data-is-talking="true"
        />\n    — Wybierzcie, zdaniem waszym, najwinniejszych i wypytajcie ich w mojej obecności. Nie chcę jednak, ażeby mnie poznali.\n  
      </p>
      <p data-index="34">
        Otworzono <span 
          className="character-highlighted" 
          data-character="Ksiaze-Ramzes" 
          data-src-listening="/Pharaon/ksiaze-ramzes-listens.mp4"
        >
          następcy tronu
        </span> izbę, w której prowadził czynności urzędnik śledczy. <span 
          className="character-highlighted" 
          data-character="Ksiaze-Ramzes" 
          data-src-listening="/Pharaon/ksiaze-ramzes-listens.mp4"
        >
          Książę
        </span> kazał mu zająć zwykłe miejsce, a sam usiadł za słupem.
      </p>
      <p data-index="35">
        Niebawem zaczęli ukazywać się pojedynczo oskarżeni. Wszyscy byli chudzi; porosły im duże włosy i brody, a oczy miały wyraz spokojnego obłąkania.
      </p>
      <p data-index="36">
        \n    — <span 
          className="character-highlighted" 
          data-character="Dutmoze" 
          data-src-listening="/Pharaon/dutmoze-listens.mp4"
        >
          Dutmoze
        </span> — rzekł urzędnik — opowiedz, jak to napadliście na dom najdostojniejszego <span 
          className="character-highlighted" 
          data-character="Ksiaze-Ramzes" 
          data-src-listening="/Pharaon/ksiaze-ramzes-listens.mp4"
        >
          erpatre
        </span>.\n  
      </p>
      <p data-index="37">
        <span 
          className="character-placeholder character-talking" 
          data-character="Dutmoze" 
          data-src-talking="/Pharaon/dutmoze-speaks.mp4" 
          data-is-talking="true"
        />\n    — Powiem prawdę, jak na sądzie Ozyrysa. Było to wieczorem tego dnia, kiedy Nil miał zacząć przybór. Moja żona mówi do mnie: „Chodź, ojcze, pójdziemy na górę, skąd prędzej można zobaczyć sygnał w Memfisie.\' Więc poszliśmy na górę, skąd łatwiej można zobaczyć sygnał w Memfisie. Wtedy do mojej żony zbliżył się jakiś żołnierz i mówi: „Pójdź ze mną w ten ogród, to znajdziemy winogron albo i co jeszcze.\' Więc moja żona poszła w ogród z owym żołnierzem, a ja wpadłem w wielki gniew i zaglądałem do nich przez mur. Czy jednak rzucali kamienie do domu <span 
          className="character-highlighted" 
          data-character="Ksiaze-Ramzes" 
          data-src-listening="/Pharaon/ksiaze-ramzes-listens.mp4"
        >
          księcia
        </span>, powiedzieć nie mogę, gdyż z powodu drzew i ciemności nic nie widziałem.\n  
      </p>
      <p data-index="38">
        \n    — A jakże mogłeś puścić żonę z żołnierzem? — spytał urzędnik.\n  
      </p>
      <p data-index="39">
        <span 
          className="character-placeholder character-talking" 
          data-character="Dutmoze" 
          data-src-talking="/Pharaon/dutmoze-speaks.mp4" 
          data-is-talking="true"
        />\n    — Za pozwoleniem waszej dostojności, a cóż ja miałem zrobić? Przeciem ja tylko chłop, a on wojownik i żołnierz jego świątobliwości…\n  
      </p>
      <p data-index="40">
        \n    — A kapłana widziałeś, który do was przemawiał?\n  
      </p>
      <p data-index="41">
        <span 
          className="character-placeholder character-talking" 
          data-character="Dutmoze" 
          data-src-talking="/Pharaon/dutmoze-speaks.mp4" 
          data-is-talking="true"
        />\n    — To nie był kapłan — odparł chłop z przekonaniem. — To musiał być sam bóg Num, bo wyszedł z pnia figowego i miał baranią głowę.\n  
      </p>
      <p data-index="42">
        \n    — A widziałeś, że miał baranią głowę?\n  
      </p>
      <p data-index="43">
        <span 
          className="character-placeholder character-talking" 
          data-character="Dutmoze" 
          data-src-talking="/Pharaon/dutmoze-speaks.mp4" 
          data-is-talking="true"
        />\n    — Za pozwoleniem, dobrze nie pamiętam, czy ja sam widziałem, czy tak mówili ludzie. Oczy zasłaniała mi troska o moją żonę.\n  
      </p>
      <p data-index="44">
        \n    — Kamienie rzucałeś do ogrodu?\n  
      </p>
      <p data-index="45">
        <span 
          className="character-placeholder character-talking" 
          data-character="Dutmoze" 
          data-src-talking="/Pharaon/dutmoze-speaks.mp4" 
          data-is-talking="true"
        />\n    — Po cóż bym rzucał, panie życia i śmierci? Gdybym trafił żonę, sobie zrobiłbym niepokój na cały tydzień, a gdyby żołnierza, dostałbym pięścią w brzuch, ażby mi język wylazł. Przeciem ja tylko chłop, a on wojownik wiecznie żyjącego pana naszego.\n  
      </p>
      <p data-index="46">
        <span 
          className="character-highlighted" 
          data-character="Ksiaze-Ramzes" 
          data-src-listening="/Pharaon/ksiaze-ramzes-listens.mp4"
        >
          Następca
        </span> wychylił się spoza kolumny. Odprowadzono <span 
          className="character-highlighted" 
          data-character="Dutmoze" 
          data-src-listening="/Pharaon/dutmoze-listens.mp4"
        >
          Dutmoze
        </span>, a wprowadzono <span 
          className="character-highlighted" 
          data-character="Anupa" 
          data-src-listening="/Pharaon/anupa-listens.mp4"
        >
          Anupa
        </span>. Był to chłop niski, na plecach miał jasne blizny od kijów.\n  
      </p>
      <p data-index="47">
        \n    — Powiedz, <span 
          className="character-highlighted" 
          data-character="Anupa" 
          data-src-listening="/Pharaon/anupa-listens.mp4"
        >
          Anupa
        </span> — zaczął znowu urzędnik — jak to było z tym napadem na ogród <span 
          className="character-highlighted" 
          data-character="Ksiaze-Ramzes" 
          data-src-listening="/Pharaon/ksiaze-ramzes-listens.mp4"
        >
          następcy tronu
        </span>?\n  
      </p>
      <p data-index="48">
        <span 
          className="character-placeholder character-talking" 
          data-character="Anupa" 
          data-src-talking="/Pharaon/anupa-speaks.mp4" 
          data-is-talking="true"
        />\n    — Oko słońca — odparł chłop — naczynie mądrości, ty wiesz najlepiej, że ja napadu nie robiłem. Tylko przyszedł do mnie sąsiad i mówi: „<span 
          className="character-highlighted" 
          data-character="Anupa" 
          data-src-listening="/Pharaon/anupa-listens.mp4"
        >
          Anupa
        </span>, chodź na górę, bo Nil przybiera.\' A ja mówię: czy aby przybiera? A on mówi: „Jesteś głupszy od osła, bo przecież osioł usłyszałby muzykę na górze, a ty nie słyszysz.\' Ja zaś odpowiadam: głupi jestem, bom się pisać nie uczył, ale za pozwoleniem, co innego jest muzyka, a co innego przybór. A on na to: „Gdyby nie było przyboru, ludzie nie mieliby z czego cieszyć się, grać i śpiewać.\' Więc poszliśmy, mówię waszej sprawiedliwości, na górę, a tam już muzykę rozpędzili i ciskają w ogród kamienie…\n  
      </p>
      <p data-index="49">
        \n    — Kto ciskał?\n  
      </p>
      <p data-index="50">
        <span 
          className="character-placeholder character-talking" 
          data-character="Anupa" 
          data-src-talking="/Pharaon/anupa-speaks.mp4" 
          data-is-talking="true"
        />\n    — Nie mogłem zmiarkować. Ludzie ci nie wyglądali na chłopów; prędzej na nieczystych paraszytów, którzy rozpruwają zmarłych do balsamowania.\n  
      </p>
      <p data-index="51">
        \n    — A kapłana widziałeś?\n  
      </p>
      <p data-index="52">
        <span 
          className="character-placeholder character-talking" 
          data-character="Anupa" 
          data-src-talking="/Pharaon/anupa-speaks.mp4" 
          data-is-talking="true"
        />\n    — Za pozwoleniem waszej czujności, to nie był kapłan, ale chyba jakiś duch, który pilnuje domu <span 
          className="character-highlighted" 
          data-character="Ksiaze-Ramzes" 
          data-src-listening="/Pharaon/ksiaze-ramzes-listens.mp4"
        >
          księcia następcy
        </span> (oby żył wiecznie!…).\n  
      </p>
      <p data-index="53">
        \n    — Dlaczego duch?\n  
      </p>
      <p data-index="54">
        <span 
          className="character-placeholder character-talking" 
          data-character="Anupa" 
          data-src-talking="/Pharaon/anupa-speaks.mp4" 
          data-is-talking="true"
        />\n    — Bo czasami tom go widział, a czasem gdzieś mi się podziewał.\n  
      </p>
      <p data-index="55">
        \n    — Może go ludzie zasłaniali?\n  
      </p>
      <p data-index="56">
        <span 
          className="character-placeholder character-talking" 
          data-character="Anupa" 
          data-src-talking="/Pharaon/anupa-speaks.mp4" 
          data-is-talking="true"
        />\n    — Z pewnością, że go czasem ludzie zasłaniali. Ale za to raz był wyższy, a inny raz niższy.\n  
      </p>
      <p data-index="57">
        \n    — Może właził na pagórek i złaził z niego?\n  
      </p>
      <p data-index="58">
        <span 
          className="character-placeholder character-talking" 
          data-character="Anupa" 
          data-src-talking="/Pharaon/anupa-speaks.mp4" 
          data-is-talking="true"
        />\n    — Bez zawodu musiał włazić i złazić, ale może wydłużał się i skracał, gdyż był to wielki cudotwórca. Ledwie rzekł: „Zaraz Nil przybierze\' — i wnet Nil zaczął przybierać.\n  
      </p>
      <p data-index="59">
        \n    — A kamienie rzucałeś, <span 
          className="character-highlighted" 
          data-character="Anupa" 
          data-src-listening="/Pharaon/anupa-listens.mp4"
        >
          Anupa
        </span>?\n  
      </p>
      <p data-index="60">
        <span 
          className="character-placeholder character-talking" 
          data-character="Anupa" 
          data-src-talking="/Pharaon/anupa-speaks.mp4" 
          data-is-talking="true"
        />\n    — Gdzież bym śmiał rzucać kamienie w ogród <span 
          className="character-highlighted" 
          data-character="Ksiaze-Ramzes" 
          data-src-listening="/Pharaon/ksiaze-ramzes-listens.mp4"
        >
          następcy tronu
        </span>?… Przecie ja prosty chłop i ręka uschłaby mi po łokieć za takie świętokradztwo.\n  
      </p>
      <p data-index="61">
        <span 
          className="character-highlighted" 
          data-character="Ksiaze-Ramzes" 
          data-src-listening="/Pharaon/ksiaze-ramzes-listens.mp4"
        >
          Książę
        </span> kazał przerwać śledztwo. A gdy wyprowadzono oskarżonych, odezwał się do urzędnika:\n  
      </p>
      <p data-index="62">
        <span 
          className="character-placeholder character-talking" 
          data-character="Ksiaze-Ramzes" 
          data-src-talking="/Pharaon/ksiaze-ramzes-speaks.mp4" 
          data-is-talking="true"
        />\n    — Więc ci ludzie należą do najwinniejszych?\n  
      </p>
      <p data-index="63">
        \n    — Rzekłeś, panie — odparł urzędnik.\n  
      </p>
      <p data-index="64">
        <span 
          className="character-placeholder character-talking" 
          data-character="Ksiaze-Ramzes" 
          data-src-talking="/Pharaon/ksiaze-ramzes-speaks.mp4" 
          data-is-talking="true"
        />\n    — W takim razie jeszcze dzisiaj trzeba uwolnić wszystkich. Ludzie nie mogą być więzieni za to, że chcieli przekonać się, czy święty Nil przybiera lub że słuchali muzyki.\n  
      </p>
      <p data-index="65">
        \n    — Najwyższa mądrość mówi przez twoje usta, <span 
          className="character-highlighted" 
          data-character="Ksiaze-Ramzes" 
          data-src-listening="/Pharaon/ksiaze-ramzes-listens.mp4"
        >
          erpatre
        </span> — rzekł urzędnik. — Kazano mi znaleźć najwinniejszych, więc wybrałem tych, jakich znalazłem. Ale nie w mojej mocy jest powrócić im wolność.\n  
      </p>
      <p data-index="66">
        <span 
          className="character-placeholder character-talking" 
          data-character="Ksiaze-Ramzes" 
          data-src-talking="/Pharaon/ksiaze-ramzes-speaks.mp4" 
          data-is-talking="true"
        />\n    — Dlaczego?\n  
      </p>
      <p data-index="67">
        \n    — Spojrzyj, najdostojniejszy, na tą skrzynię. Jest ona pełna papirusów, na których spisano akta sprawy. Sędzia z Memfisu co dzień otrzymuje raporty o jej przebiegu i donosi jego świątobliwości. W cóż obróciłaby się praca tylu uczonych pisarzy i wielkich mężów, gdyby oskarżonych uwolnić?\n  
      </p>
      <p data-index="68">
        <span 
          className="character-placeholder character-talking" 
          data-character="Ksiaze-Ramzes" 
          data-src-talking="/Pharaon/ksiaze-ramzes-speaks.mp4" 
          data-is-talking="true"
        />\n    — Ależ oni są niewinni! — zawołał <span 
          className="character-highlighted" 
          data-character="Ksiaze-Ramzes" 
          data-src-listening="/Pharaon/ksiaze-ramzes-listens.mp4"
        >
          książę
        </span>.\n  
      </p>
      <p data-index="69">
        \n    — Napad był, a więc było przestępstwo. Gdzie jest przestępstwo, muszą być przestępcy, a kto raz dostał się w ręce władzy i jest opisany w aktach, nie może odejść bez jakiegoś rezultatu. W szynku człowiek pije i płaci; na jarmarku coś sprzedaje i otrzymuje; w polu sieje i zbiera; w grobach dostaje błogosławieństwa od zmarłych przodków. Jakim więc sposobem ktoś, przyszedłszy do sądu, wróciłby z niczym jak podróżny, który zatrzymuje się w połowie swej drogi i zwraca stopy do domu, nie osiągnąwszy celu?\n  
      </p>
      <p data-index="70">
        <span 
          className="character-placeholder character-talking" 
          data-character="Ksiaze-Ramzes" 
          data-src-talking="/Pharaon/ksiaze-ramzes-speaks.mp4" 
          data-is-talking="true"
        />\n    — Mądrze mówisz — odparł <span 
          className="character-highlighted" 
          data-character="Ksiaze-Ramzes" 
          data-src-listening="/Pharaon/ksiaze-ramzes-listens.mp4"
        >
          następca
        </span>. — Powiedz mi jednak, czy i jego świątobliwość nie miałby prawa uwolnić tych ludzi?\n  
      </p>
      <p data-index="71">
        Urzędnik złożył ręce na krzyż i schylił głowę.
      </p>
      <p data-index="72">
        \n    — On, równy bogom, wszystko, co chce, uczynić może: uwolnić oskarżonych, nawet skazanych, a nawet zniszczyć akta sprawy, co spełnione przez zwykłego człowieka byłoby świętokradztwem.\n  
      </p>
      <p data-index="73">
        <span 
          className="character-highlighted" 
          data-character="Ksiaze-Ramzes" 
          data-src-listening="/Pharaon/ksiaze-ramzes-listens.mp4"
        >
          Książę
        </span> pożegnał urzędnika i polecił nadzorcy, ażeby na jego koszt lepiej karmiono oskarżonych o napad. Następnie, rozdrażniony, popłynął na drugą stronę ciągle rozszerzającej się rzeki, do pałacu, ażeby prosić <span 
          className="character-highlighted" 
          data-character="Ramzes-XII" 
          data-src-listening="/Pharaon/ramzes-xii-listens.mp4"
        >
          faraona
        </span> o umorzenie nieszczęsnej sprawy.\n  
      </p>
      <p data-index="74">
        Tego jednak dnia jego świątobliwość miał dużo ceremonii religijnych i naradę z ministrami, więc <span 
          className="character-highlighted" 
          data-character="Ksiaze-Ramzes" 
          data-src-listening="/Pharaon/ksiaze-ramzes-listens.mp4"
        >
          następca
        </span> nie mógł się z nim widzieć. Wówczas <span 
          className="character-highlighted" 
          data-character="Ksiaze-Ramzes" 
          data-src-listening="/Pharaon/ksiaze-ramzes-listens.mp4"
        >
          książę
        </span> udał się do <span 
          className="character-highlighted" 
          data-character="Wielki-pisarz" 
          data-src-listening="/Pharaon/wielki-pisarz-listens.mp4"
        >
          wielkiego pisarza
        </span>, który po ministrze wojny najbardziej znaczył we dworze. Stary ten urzędnik, kapłan jednej ze świątyń w Memfis, przyjął <span 
          className="character-highlighted" 
          data-character="Ksiaze-Ramzes" 
          data-src-listening="/Pharaon/ksiaze-ramzes-listens.mp4"
        >
          księcia
        </span> grzecznie, ale zimno, a wysłuchawszy go, odparł:
      </p>
      <p data-index="75">
        <span 
          className="character-placeholder character-talking" 
          data-character="Wielki-pisarz" 
          data-src-talking="/Pharaon/wielki-pisarz-speaks.mp4" 
          data-is-talking="true"
        />\n    — Dziwno mi, że wasza dostojność podobnymi sprawami chcesz niepokoić naszego pana. Jest to to samo, co gdybyś prosił o nietępienie szarańczy, która spadła na pole.\n  
      </p>
      <p data-index="76">
        <span 
          className="character-placeholder character-talking" 
          data-character="Ksiaze-Ramzes" 
          data-src-talking="/Pharaon/ksiaze-ramzes-speaks.mp4" 
          data-is-talking="true"
        />\n    — Ależ to są ludzie niewinni!…\n  
      </p>
      <p data-index="77">
        <span 
          className="character-placeholder character-talking" 
          data-character="Wielki-pisarz" 
          data-src-talking="/Pharaon/wielki-pisarz-speaks.mp4" 
          data-is-talking="true"
        />\n    — My, dostojny panie, wiedzieć o tym nie możemy, gdyż o winie lub niewinności rozstrzyga prawo i sąd. Jedno dla mnie jest pewnym, że państwo nie może ścierpieć, ażeby wpadano do czyjegoś ogrodu, a tym bardziej, ażeby podnoszono rękę na własność <span 
          className="character-highlighted" 
          data-character="Ksiaze-Ramzes" 
          data-src-listening="/Pharaon/ksiaze-ramzes-listens.mp4"
        >
          następcy tronu
        </span>.\n  
      </p>
      <p data-index="78">
        <span 
          className="character-placeholder character-talking" 
          data-character="Ksiaze-Ramzes" 
          data-src-talking="/Pharaon/ksiaze-ramzes-speaks.mp4" 
          data-is-talking="true"
        />\n    — Sprawiedliwie mówisz, ale — gdzież są winni?… — spytał <span 
          className="character-highlighted" 
          data-character="Ksiaze-Ramzes" 
          data-src-listening="/Pharaon/ksiaze-ramzes-listens.mp4"
        >
          książę
        </span>.\n  
      </p>
      <p data-index="79">
        <span 
          className="character-placeholder character-talking" 
          data-character="Wielki-pisarz" 
          data-src-talking="/Pharaon/wielki-pisarz-speaks.mp4" 
          data-is-talking="true"
        />\n    — Gdzie nie ma winnych, muszą być przynajmniej ukarani. Nie wina, ale kara następująca po zbrodni uczy innych, że tego spełniać nie wolno.\n  
      </p>
      <p data-index="80">
        <span 
          className="character-placeholder character-talking" 
          data-character="Ksiaze-Ramzes" 
          data-src-talking="/Pharaon/ksiaze-ramzes-speaks.mp4" 
          data-is-talking="true"
        />\n    — Widzę — przerwał <span 
          className="character-highlighted" 
          data-character="Ksiaze-Ramzes" 
          data-src-listening="/Pharaon/ksiaze-ramzes-listens.mp4"
        >
          następca
        </span> — że wasza dostojność nie poprzesz mojej prośby u jego świątobliwości.\n  
      </p>
      <p data-index="81">
        \n    — Mądrość płynie z ust twoich, <span 
          className="character-highlighted" 
          data-character="Ksiaze-Ramzes" 
          data-src-listening="/Pharaon/ksiaze-ramzes-listens.mp4"
        >
          erpatre
        </span> — odpowiedział dygnitarz. — Nigdy nie potrafię udzielać panu memu rady, która powagę władzy naraziłaby na szwank…\n  
      </p>
      <p data-index="82">
        <span 
          className="character-highlighted" 
          data-character="Ksiaze-Ramzes" 
          data-src-listening="/Pharaon/ksiaze-ramzes-listens.mp4"
        >
          Książę
        </span> wrócił do siebie zbolały i zdumiony. Czuł, że kilkuset ludziom dzieje się krzywda, i widział, że ratować ich nie może. Jak nie potrafiłby wydobyć człowieka, na którego upadł obelisk albo kolumna świątyni.\n  
      </p>
      <p data-index="83">
        „Za słabe są moje ręce do podniesienia tego gmachu\' — myślał <span 
          className="character-highlighted" 
          data-character="Ksiaze-Ramzes" 
          data-src-listening="/Pharaon/ksiaze-ramzes-listens.mp4"
        >
          książę
        </span> z uciskiem w duszy. Pierwszy raz uczuł, że od jego woli jest jakaś nieskończenie większa siła: interes państwa, który uznaje nawet wszechmocny <span 
          className="character-highlighted" 
          data-character="Ramzes-XII" 
          data-src-listening="/Pharaon/ramzes-xii-listens.mp4"
        >
          faraon
        </span>, a przed którym ugiąć się musi <span 
          className="character-highlighted" 
          data-character="Ksiaze-Ramzes" 
          data-src-listening="/Pharaon/ksiaze-ramzes-listens.mp4"
        >
          on, następca
        </span>!
      </p>
      <p data-index="84">
        Zapadła noc. <span 
          className="character-highlighted" 
          data-character="Ksiaze-Ramzes" 
          data-src-listening="/Pharaon/ksiaze-ramzes-listens.mp4"
        >
          Ramzes
        </span> nie kazał służbie nikogo przyjmować i samotny chodził po tarasie swojej willi, dumając:
      </p>
      <p data-index="85">
        „Straszna rzecz!… Tam rozstąpiły się przede mną niezwyciężone pułki <span 
          className="character-highlighted" 
          data-character="Nitager" 
          data-src-listening="/Pharaon/nitager-listens.mp4"
        >
          Nitagera
        </span>, a tu — nadzorca więzienia, urzędnik śledczy i <span 
          className="character-highlighted" 
          data-character="Wielki-pisarz" 
          data-src-listening="/Pharaon/wielki-pisarz-listens.mp4"
        >
          wielki pisarz
        </span> zabiegają mi drogę… Czymże oni są?… Nędznymi sługami mojego <span 
          className="character-highlighted" 
          data-character="Ramzes-XII" 
          data-src-listening="/Pharaon/ramzes-xii-listens.mp4"
        >
          ojca
        </span> (oby żył wiecznie!), który każdej chwili może ich strącić do rzędu niewolników i zesłać w kamieniołomy. Ale dlaczego <span 
          className="character-highlighted" 
          data-character="Ramzes-XII" 
          data-src-listening="/Pharaon/ramzes-xii-listens.mp4"
        >
          ojciec mój
        </span> nie miałby ułaskawić niewinnych?… Państwo tak chce!… I cóż to jest państwo?… Co ono jada, gdzie sypia, gdzie jego ręce i miecz, którego się wszyscy boją?…\'
      </p>
      <p data-index="86">
        Spojrzał w ogród i między drzewami, na szczycie wzgórza, zobaczył dwie olbrzymie sylwetki pylonów, na których płonęły kagańce straży. Przyszło mu na myśl, że ta straż nigdy nie śpi i że pylony nigdy nie jedzą, a jednak są. Odwieczne pylony, potężne jak mocarz, który je wznosił, Ramzes Wielki.
      </p>
      <p data-index="87">
        Poruszyć te gmachy i setki im podobnych; zmylić tą straż i tysiące innych, które czuwają nad bezpieczeństwem Egiptu; okazać nieposłuszeństwo prawom, które pozostawił Ramzes Wielki i inni, jeszcze więksi przed nim mocarze, a które dwadzieścia dynastii uświęciło swoim poszanowaniem…
      </p>
      <p data-index="88">
        W duszy <span 
          className="character-highlighted" 
          data-character="Ksiaze-Ramzes" 
          data-src-listening="/Pharaon/ksiaze-ramzes-listens.mp4"
        >
          księcia
        </span>, pierwszy raz w życiu, poczęło zarysowywać się jakieś niejasne, ale olbrzymie pojęcie — państwa. Państwo jest to coś wspanialszego od świątyni w Tebach, coś większego od piramidy Cheopsa, coś dawniejszego od podziemi Sfinksa, coś trwalszego od granitu. W tym niezmiernym, choć niewidzialnym gmachu ludzie są jako mrówki w szczelinie skalnej, a <span 
          className="character-highlighted" 
          data-character="Ramzes-XII" 
          data-src-listening="/Pharaon/ramzes-xii-listens.mp4"
        >
          faraon
        </span> jak podróżny architekt, który ledwie zdąży osadzić jeden głaz w ścianie i już odchodzi. A ściany rosną od pokolenia do pokolenia i budowa trwa dalej.
      </p>
      <p data-index="89">
        Jeszcze nigdy on, syn królewski, nie czuł tak swojej małości jak w tej chwili, kiedy jego wzrok wśród nocy błądził ponad Nilem, między pylonami zamku <span 
          className="character-highlighted" 
          data-character="Ramzes-XII" 
          data-src-listening="/Pharaon/ramzes-xii-listens.mp4"
        >
          faraona
        </span> i niewyraźnymi, lecz przepotężnymi sylwetkami memfijskich świątyń.
      </p>
      <p data-index="90">
        Wtem, spomiędzy drzew, których konary dotykały tarasu, odezwał się głos:
      </p>
      <p data-index="91">
        \n    — Znam twoją troskę i błogosławię cię. Sąd nie uwolni oskarżonych chłopów. Ale sprawa ich może upaść i wrócą w pokoju do swych domów, jeżeli dozorca twego folwarku nie będzie popierał skargi o napad.\n  
      </p>
      <p data-index="92">
        <span 
          className="character-placeholder character-talking" 
          data-character="Ksiaze-Ramzes" 
          data-src-talking="/Pharaon/ksiaze-ramzes-speaks.mp4" 
          data-is-talking="true"
        />\n    — Więc to mój dozorca podał skargę?… — spytał zdziwiony <span 
          className="character-highlighted" 
          data-character="Ksiaze-Ramzes" 
          data-src-listening="/Pharaon/ksiaze-ramzes-listens.mp4"
        >
          książę
        </span>.\n  
      </p>
      <p data-index="93">
        \n    — Prawdę rzekłeś. On podał ją w twoim imieniu. Ale jeżeli nie przyjdzie na sąd, nie będzie pokrzywdzonego; a gdzie nie ma pokrzywdzonego, nie ma przestępstwa.\n  
      </p>
      <p data-index="94">
        Krzaki zaszeleściły.
      </p>
      <p data-index="95">
        <span 
          className="character-placeholder character-talking" 
          data-character="Ksiaze-Ramzes" 
          data-src-talking="/Pharaon/ksiaze-ramzes-speaks.mp4" 
          data-is-talking="true"
        />\n    — Stójże! — zawołał <span 
          className="character-highlighted" 
          data-character="Ksiaze-Ramzes" 
          data-src-listening="/Pharaon/ksiaze-ramzes-listens.mp4"
        >
          Ramzes
        </span>. — Kto jesteś?…\n  
      </p>
      <p data-index="96">
        Nikt nie odpowiedział. Tylko zdawało się <span 
          className="character-highlighted" 
          data-character="Ksiaze-Ramzes" 
          data-src-listening="/Pharaon/ksiaze-ramzes-listens.mp4"
        >
          księciu
        </span>, że w smudze światła pochodni, palącej się na pierwszym piętrze, mignęła naga głowa i skóra pantery.
      </p>
      <p data-index="97">
        <span 
          className="character-placeholder character-talking" 
          data-character="Ksiaze-Ramzes" 
          data-src-talking="/Pharaon/ksiaze-ramzes-speaks.mp4" 
          data-is-talking="true"
        />\n    — Kapłan?… — szepnął <span 
          className="character-highlighted" 
          data-character="Ksiaze-Ramzes" 
          data-src-listening="/Pharaon/ksiaze-ramzes-listens.mp4"
        >
          następca
        </span>. — Dlaczego on kryje się?…\n  
      </p>
      <p data-index="98">
        Lecz w tej chwili przyszło mu na myśl, że ów kapłan mógłby ciężko odpowiadać za udzielanie rad tamujących wymiar sprawiedliwości.
      </p>
    </section>
  );
};

export default Chapter11;
