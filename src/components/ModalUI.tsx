  ...
  return (
    <div className={cn("fixed inset-0 flex items-center justify-center", hideOverlay ? "pointer-events-none" : "bg-black/50")} onClick={preventClickOutside ? undefined : onClose}>
      <div className="flex flex-row gap-2 justify-center mx-auto pl-2 pr-2 md:pr-0 xl:px-4 md:pl-4 max-w-[150rem] w-full h-full">
        {layoutView && <div id="left-notes-blank" className={cn("hidden md:block md:flex-1 max-w-[700px] xl:order-1", "lg:order-2 lg:flex-2")} />}
        <div
          className={cn(
            `bg-white dark:bg-gray-800 rounded-lg overflow-hidden ${widthClasses[width]} ${heightClasses[height]} w-full flex flex-col shadow-xl ${className} pointer-events-auto`,
            "xl:flex-1 xl:max-w-[700px] xl:order-3",
            "lg:order-1 lg:max-w-[700px] lg:flex-1 pointer-events-all",
          )}
        ...
        {layoutView && <div id="right-notes-blank" className={cn("hidden xl:block xl:flex-2 max-w-[900px] xl:order-2 ", "")} />}
      </div>
    </div>
  );
};
